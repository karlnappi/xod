...
import { foldEither, explodeEither, catMaybies } from 'xod-func-tools';
...
const checkResolvedTypesForConflicts = R.ifElse(
  ...
  () => Either.Left(new Error(CONST.ERROR.ALL_TYPES_MUST_BE_RESOLVED)),
      ...
      R.map(([abstractNodeId, pinTypes]) => {
        ...
        if (R.isEmpty(matchingSpecializations)) {
          return Either.Left(
            new Error(
              Utils.formatString(
                CONST.ERROR.CANT_FIND_SPECIALIZATIONS_FOR_ABSTRACT_PATCH,
        ...
        if (matchingSpecializations.length > 1) {
          return Either.Left(
            new Error(
              Utils.formatString(
                CONST.ERROR.CONFLICTING_SPECIALIZATIONS_FOR_ABSTRACT_PATCH,
                {
                  patchPath: Patch.getPatchPath(abstractPatch),
                  conflictingSpecializations: matchingSpecializations
                    .map(R.unary(Patch.getPatchPath))
                    .join(', '),
                }
              )
            )
